...
//模拟10w人基本用户，每天新增1k人左右，每天活跃用户1w左右，新增100左右,uid多1k,
//1w左右start，启动日志
//2w左右资源列表loading
//20w左右display，浏览日志
//2w左右newsdetail，资源详情
//5k左右点赞，1k左右收藏，500左右评论
//300左右search
...
public class ProduceLogs {
    ...
    private final static Logger logger = LoggerFactory.getLogger(ProduceLogs.class);
    ...
    private static int[] todayUids = genRandomArray(100000,9000+(int)(Math.random()*2000));
    private static int[] addMids = genRandomArray(100000,1000);
    // 从 m个整数中随机抽取 n个整数，条件: m > n

    public static void main(String[] args) {
        for (int i = 0; i < addMids.length; i++) {
            addMids[i]+=100000;
        }
